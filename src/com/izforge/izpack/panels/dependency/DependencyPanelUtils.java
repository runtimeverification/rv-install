...
public class DependencyPanelUtils {
     ...
     *
     * @param installData Current installer
     * @return Current panel ID
     */
    protected static String getId(Panel panel) {
     ...
     *
     * @param installData Current installer, id The ID of the current dependency
     * @return Current panel dependencies, as ArrayList
     */
    protected static ArrayList<String> getDependencies(InstallData installData, String id) {
        int i = 1;
        ArrayList<String> dependencyList = new ArrayList<String>();
        String dependencyExecutable = installData.getVariable("DependencyPanel." + id + ".exe." + i);
        while (dependencyExecutable != null) {
            dependencyList.add(dependencyExecutable);
            i++;
            dependencyExecutable = installData.getVariable("DependencyPanel." + id + ".exe." + i);
        }
     ...
     */
    protected static ArrayList<DependencyPanelTest> getDependencyTests(InstallData installData, String id) {
        int i = 1;
        ArrayList<DependencyPanelTest> testList = new ArrayList<DependencyPanelTest>();
        String testCommand = installData.getVariable("DependencyPanel." + id + ".test." + i + ".command");
        while (testCommand != null) {
            String testOutput = installData.getVariable("DependencyPanel." + id + ".test." + i + ".output_contains");
            testList.add(new DependencyPanelTest(testCommand, testOutput));
            i++;
            testCommand = installData.getVariable("DependencyPanel." + id + ".test." + i + ".command");
        }
        return testList;
    }
     ...
     *
     * @param installData Current installer data
     * @return Current panel's dependency install site
     */
    protected static String getDependencySite(InstallData installData, String id) {
        final String dependencySite = installData.getVariable("DependencyPanel." + id + ".site");
     ...
     */
    protected static String getDependencyHTML(String id, Resources resources) {
        final String dependencyHTML;
        try {
            dependencyHTML = resources.getString("DependencyPanel." + id + ".html");
        }
        catch (Exception ex) {
     ...
     */
    protected static String getDependencyText(String id, Resources resources) {
        return removeHTML(getDependencyHTML(id, resources));
    }
